...
### 2.4 自然光 vs X光：两种渲染原理的「致命差异」

这一节是整个 SPAGS 方法**最核心的动机**。理解了这里，你就理解了我们为什么要做剪枝而不是密化。

#### 2.4.1 自然光 3DGS：Alpha 混合渲染

标准 3DGS [Kerbl et al., SIGGRAPH 2023] **渲染自然光场景**时，用的是 **Alpha 混合（α-blending）**：

```
像素颜色 = Σᵢ (αᵢ × cᵢ × Tᵢ)

其中：
  αᵢ = 第 i 个高斯的透明度（0=透明，1=不透明）
  cᵢ = 第 i 个高斯的颜色
  Tᵢ = Πⱼ₌₁ⁱ⁻¹ (1 - αⱼ)   ← 透过度：前面高斯挡住的光线比例
```

**关键性质：Alpha 混合有「遮挡关系」**

想象你透过一扇窗户看远处的山：
- 窗户玻璃挡住了 80% 的光（α=0.8）→ 你只能看到 20% 的山
- **前面的物体会遮挡后面的物体**——这是自然光场景的物理规律

这意味着什么？**对一个像素，只有最前面的几个高斯（透过度 T 不为 0 的那些）能收到梯度更新**。后面被遮挡的高斯几乎接收不到梯度信号，因为它们的贡献被前面的挡住了。
...
结果：只有「表面」高斯在学，「内部」高斯被挡了
```

**这对自适应控制的影响**：
- 梯度大的地方 → 需要加点 → 加到「表面」附近
- 稀疏区域（远处背景、物体之间的空隙）→ 邻近分数高 → FSGS 的邻近密化在这里有效
- 自然场景本质上是「面」的集合，空间大部分是空的

#### 2.4.2 X 光 3DGS：比尔-朗伯辐射传输

R²-Gaussian [Hu et al., NeurIPS 2024] **渲染 X 光场景**时，用的是 **比尔-朗伯定律（Beer-Lambert Law）**：
...
和 Alpha 混合有本质区别：
- X 光是**穿透性辐射**，不是反射光
- **所有在射线路径上的高斯都贡献**——前面的高斯不会挡住后面的
- 密度是**加法关系**：ρ₁ + ρ₂ + ρ₃ + ... 而不是遮挡关系
...
射线方向 →  ●(前)  ●  ●  ●  ●(后)
   ...
   阻挡？   ×     ×  ×  ×  ×    ← 全都不阻挡！
            │     │  │  │  │
  梯度大小:  大    大  大  大  大   ← 全都收到梯度！

结果：所有高斯都在学，越挤的地方梯度越大
```

#### 2.4.3 为什么这导致我们需要「剪枝」而非「密化」

现在我们把两种渲染方式的差异整理成一张表：

| 特性 | 自然光 3DGS (Alpha 混合) | X 光 3DGS (比尔-朗伯) |
|------|--------------------------|----------------------|
| **渲染公式** | `C = Σ αcT` | `I = I₀·exp(-Σ ρℓ)` |
| **遮挡效应** | ✅ 有（前面挡后面） | ❌ 无（全部穿透） |
| **梯度分布** | 集中在表面高斯 | 均匀分布在所有高斯 |
| **场景占空比** | 稀疏（物体只占小部分空间） | 稠密（整个空间都有物质） |
| **核心问题** | 覆盖不足（空洞、背面） | **冗余过多（堆叠、拥挤）** |
| **FSGS 策略** | ✅ 在稀疏区域**加点** 有效 | ❌ 在稀疏区域加点 → 加剧堆叠 |
| **正确策略** | 邻近密化（FSGS） | **邻近剪枝（我们的 GAP）** |

**本质原因更深入地看**：

**在自然光 3DGS 中**：
- 梯度信号天然是**稀疏的**——只有可见表面有梯度
- 3-5 层高斯就覆盖了所有梯度信息
- 多点虽无大害，但浪费计算
- 稀疏区域（大间隔）指示了需要加点的位置 ✅

**在 X 光 3DGS 中**：
- 梯度信号是**稠密的**——所有穿过的点都有梯度
- 骨头边界有 100+ 层高斯，**每个都收到梯度**
- 每个高斯都想降低自己的密度来拟合投影 → 「内卷」
- 3DGS 的「哪里梯度大就在哪里加点」在 CT 上形成了**正反馈循环**：
  ```
  骨头边界梯度大 → 加点 → 更挤了 → 梯度仍然大 → 再加点 → ⋯⋯
  ```
- 最终结果是：骨头边界有成千上万个冗余高斯互相竞争

**这解释了为什么 GAP 能贡献 +0.42 dB 的最大提升**：
- GAP 是唯一直接针对「冗余问题」的模块
- 它打破了「梯度大→加点→更挤」的正反馈循环
- 在 3DGS 的标准流程之上增加了一个**负反馈机制**：密化过头了就剪掉

#### 2.4.4 文献支撑

| 文献 | 来源 | 与我们工作的关系 |
|------|------|-----------------|
| **3D Gaussian Splatting** [Kerbl et al., 2023] | SIGGRAPH 2023 | 定义了 Alpha 混合渲染 + 自适应密度控制的基础框架 |
| **R²-Gaussian** [Hu et al., 2024] | NeurIPS 2024 | 首次将 3DGS 扩展到 X 光 CT NVS，使用比尔-朗伯渲染 |
| **FSGS** [Zhu et al., 2024] | ECCV 2024 | 提出邻近密化策略（在稀疏区域加点），在自然光场景有效 |
| **Compact 3DGS** [Lu et al., 2024] | ECCV 2024 | 提出通过剪枝/聚类降低 3DGS 内存，和我们的 GAP 目标不同但相关 |
| **Beer-Lambert Law** | 物理学 | X 光穿过的基本物理规律，定义了密度是加性而非遮挡性的 |

**核心参考文献**：
```
@inproceedings{kerbl20233d,
  author = {Kerbl, Bernhard and Kopanas, Georgios and Leimk{\"u}hler, Thomas and Drettakis, George},
  title = {3D {G}aussian {S}platting for {R}eal-{T}ime {R}adiance {F}ield {S}ynthesis},
  booktitle = {SIGGRAPH},
  year = {2023}
}

@inproceedings{hu2024r2,
  author = {Hu, Ruyi and others},
  title = {R{\textsuperscript{2}}-{G}aussian: {R}ectilinear {R}adiative {G}aussian {S}platting},
  booktitle = {NeurIPS},
  year = {2024}
}

@inproceedings{zhu2024fsgs,
  author = {Zhu, Zehao and others},
  title = {{FSGS}: {R}eal-{T}ime {F}ew-shot {V}iew {S}ynthesis using {G}aussian {S}platting},
  booktitle = {ECCV},
  year = {2024}
}
```
...
> **自然光 3DGS 的问题是「哪里缺了点」→ 用邻近密化（FSGS 做法）**
> **X 光 3DGS 的问题是「哪里点太多了」→ 用邻近剪枝（我们的 GAP 做法）**
>
> 两者差异的根本来源是：Alpha 混合有**遮挡**，比尔-朗伯没有。就这么简单。
